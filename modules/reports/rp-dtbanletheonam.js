...
module.exports = function(router){
	var rpt = new controller(router,"dtbanletheonam",function(req,callback){
		var condition = req.query;
		if( !condition.tu_nam || !condition.den_nam ){
			return callback("Báo cáo này yêu cầu các tham số:tu_nam,den_nam");
		}
		var query ={id_app:condition.id_app};
		var tu_ngay = new Date(condition.tu_nam,0,1)
		var den_ngay = new Date(condition.den_nam,11,31)
		...
		databanle(query,function(error,rs){
			...
			async.map(rs,function(r,callback){
				...
				if(error) return callback(error);
				rs.groupBy('nam',[{name:'t_so_luong',value:'t_so_luong'},{name:'t_tien_hang',value:'t_tien_hang'},{name:'t_tien_ck',value:'t_tien_ck'}
