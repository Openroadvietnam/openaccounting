...
var kbmlcttgt = require("../../models/kbm_lcttgt");
...
module.exports = function(router){
	var rpt = new controller(router,"lcttgt",function(req,callback){
		var query = req.query;
		...
		var report;
		kbmlcttgt.find({id_app:query.id_app},function(error,kbm){
			if(error){
			...
			async.map(report,function(r,c1){
				...
				if(r.cach_tinh=='1'){
					...
					if(r.cach_tinh=='2'){
						...
						if(r.cach_tinh=='3' || r.cach_tinh=='4'||r.cach_tinh=='5'){
							async.parallel({
								//tinh ky truoc
								ky_truoc:function(callback){
									var condition = {
									...
									dktk(condition,function(error,rs){
										...
										if(r.cach_tinh=='3'){//tinh theo so du
											...
											if(r.so_kt<0 && r.khong_am){
												r.so_kt =0;
											}
											if(r.so_kt_nt<0 && r.khong_am){
												r.so_kt_nt =0;
											}
								...
								ky_nay:function(callback){
									...
									dktk(condition,function(error,rs){
										...
										if(r.cach_tinh=='3'){//tinh theo so du
											...
											if(r.so_kn<0 && r.khong_am){
												r.so_kn =0;
											}
											if(r.so_kn_nt<0 && r.khong_am){
												r.so_kn_nt =0;
											}
							...
							async.parallel({
								...
								ky_truoc:function(callback){
									...
									if(r.cach_tinh=='6' || r.cach_tinh=='7'){//tinh theo so du  hoặc du no
										var tk_no = r.tk_no;
										if(tk_no && tk_no.length==1 && tk_no[0]=="")
											tk_no = null;
										condition.tk = tk_no;
									...
									cktk(condition,function(error,rs){
										if(error) return callback(error);
										if(r.cach_tinh=='6'){//tinh theo so du
											r.so_kt = rs.csum('du_no00')-rs.csum('du_co00');
											r.so_kt_nt = rs.csum('du_no_nt00')-rs.csum('du_co_nt00');
											if(r.phan_loai=='1'){//chi
												r.so_kt =-r.so_kt;
										...
										}else if(r.cach_tinh=='7'){//tinh theo so du no
											r.so_kt = rs.csum('du_no00');
											r.so_kt_nt = rs.csum('du_no_nt00');
										}else{//tinh theo so du co
											r.so_kt = rs.csum('du_co00');
